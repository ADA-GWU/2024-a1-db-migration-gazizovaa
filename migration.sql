...
ALTER TABLE INTERESTS
ALTER COLUMN INTERESTS TYPE VARCHAR(255)[]
USING ARRAY[INTERESTS];

SELECT STUDENT_ID, ARRAY_AGG(INTERESTS) AS INTERESTS
FROM INTERESTS
GROUP BY STUDENT_ID
ORDER BY STUDENT_ID;

SELECT STUDENT_ID, ARRAY(SELECT DISTINCT INTERESTS FROM INTERESTS WHERE STUDENT_ID = t2.STUDENT_ID) AS INTERESTS
FROM (SELECT DISTINCT STUDENT_ID FROM INTERESTS) AS t2
ORDER BY STUDENT_ID;
